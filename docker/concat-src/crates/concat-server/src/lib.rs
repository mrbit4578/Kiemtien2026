...
pub struct Config {
    ...
    /// that started it can learn, through [`Server::token`].
    ///
    /// The token travels in cleartext: anyone who can read the network
    /// path learns it from a single connection's first message. A bind
    /// off loopback is therefore only as safe as the network between the
    /// server and its clients; prefer loopback plus an SSH tunnel, or a
    /// reverse proxy that terminates TLS, over exposing the port.
