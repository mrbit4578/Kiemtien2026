        ...
        .spawn(move || {
            runtime.block_on(async move {
                let Ok(listener) = tokio::net::TcpListener::from_std(listener) else {
                    eprintln!("the gRPC server could not take over its listener");
                    return;
                };
                ...
                // A failed serve used to die silently here, leaving clients
                // unable to connect with no diagnostic anywhere.
                if let Err(error) = tonic::transport::Server::builder()
                    .add_service(service)
                    .serve_with_incoming_shutdown(incoming, stopped)
                    .await
                {
                    eprintln!("the gRPC server stopped with an error: {error}");
                }
            });
        })
