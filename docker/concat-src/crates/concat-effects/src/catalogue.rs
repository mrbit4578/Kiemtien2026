...
impl Catalogue {
    ...
    /// Every transition's `fallback` must name a package id or alias in
    /// this catalogue: a dangling name degrades to nothing where the
    /// shader cannot run. Checked here, not in the manifest, because the
    /// name may belong to a package that loads later.
    pub fn check_fallbacks(&self) -> Vec<String> {
        let mut failures = Vec::new();
        for package in &self.packages {
            if package.kind() != Kind::Transition {
                continue;
            }
            let fallback = package.transition_fallback();
            if self.get(fallback).is_none() {
                failures.push(format!(
                    "{}: fallback `{fallback}` matches no package id or alias",
                    package.id()
                ));
            }
        }
    ...
    fn compose(&self, kinds: &[Kind], applied: &[AppliedFilter], skip_shaders: bool) -> String {
        ...
        for applied in applied.iter().filter(|applied| applied.enabled) {
            ...
            match package.ffmpeg_fragment(&applied.params, fragments.len()) {
                ...
                // the fragment rather than export a broken graph, but say
                // so: a silently vanishing effect is a mystery otherwise.
                Err(error) => {
                    eprintln!(
                        "concat-effects: dropping the `{}` fragment: {error}",
                        package.id()
                    );
                }
            }
