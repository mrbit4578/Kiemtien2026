...
/// depth short of a nested loop or a switch, whose breaks are their own:
/// a `break` inside a `switch` case only leaves the switch.
fn breaks(block: &naga::Block) -> bool {
    block.iter().any(|statement| match statement {
        ...
        // A switch's breaks are the switch's own; they never leave the loop.
        naga::Statement::Switch { .. } => false,
        _ => false,
    })
}
...
"#,
    ...
    fn a_hostile_shader_is_refused_at_load() {
        ...
        // A `break` inside a `switch` only leaves the switch: it must not
        // count as the loop's way out, or the loop below hangs the device.
        let switch_break = Shader::compile(
            &manifest,
            "fn effect(uv: vec2<f32>) -> vec4<f32> { var c = sample(uv); loop { switch (1) { case 1: { break; } default: {} } } return c; }",
        );
        assert!(
            switch_break.unwrap_err().contains("no break"),
            "a break inside a switch is not a way out of the loop"
        );
