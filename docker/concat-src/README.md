# VENDORED SNAPSHOT — đọc trước

**Concat 0.2.3**, license **AGPL-3.0-or-later** (upstream: `jub0t/Concat`).
Snapshot này được vendor vào repo Kiemtien2026 (thư mục `src/` của upstream,
đã loại `target/`) để Docker build **deterministic** — image production luôn
build từ đúng snapshot đã kiểm chứng. Kiemtien2026 chỉ dùng `concat-cli serve`
(headless JSON-RPC); xem `docs/concat-render.md` ở repo gốc.

## Kaizen fixes đã áp dụng trong snapshot này (audit 2026-09-23)

0 Critical · 1 High · ~12 Medium · 20+ Low. **Compile-verified**:
`cargo test -p concat-effects` → 50 passed, 0 failed.

Security:
1. **Shader loop-guard bypass (HIGH)** — `concat-effects/src/shader.rs`: `break`
   trong `switch` bị tính nhầm là lối thoát của loop (trong WGSL nó chỉ thoát
   switch) → package độc có thể treo GPU. Đã fix + regression test.
2. **Expression evaluator panics (MEDIUM)** — `concat-effects/src/expr.rs`:
   `abs(i64::MIN)`, `+`/`-`/`*` tràn số, `-n` tràn → dùng `checked_*`, trả
   `EvalError`. Có regression tests.
3. **Non-finite values (MEDIUM)** — `1/0`, `sqrt(-1)` tạo `inf`/`NaN` lọt vào
   FFmpeg chain; giờ `Expr::eval` từ chối ngay tại load.
4. **`if()` lazy (LOW)** — trước đây evaluate cả hai nhánh; giờ chỉ nhánh được chọn.
5. **Token doc gây hiểu lầm (MEDIUM)** — `concat-server`: bind ngoài loopback
   mà không có TLS là không an toàn; đã viết lại doc.
6. **Dangling transition fallback (MEDIUM)** — `concat.whip-pan` khai báo
   fallback `slideleft` (tên xfade FFmpeg, không phải package id) → sửa thành
   `cross-fade` + thêm `Catalogue::check_fallbacks()` và test.

Robustness:
7. **Poisoned mutex giết audio decode worker (HIGH)** — `concat-host`:
   `.wait().unwrap()` / `.lock().unwrap()` → giờ recover guard.
8. **gRPC serve errors chết im lặng (MEDIUM)** — giờ `eprintln!` diagnostic.
9. **Hostile font crash title render (MEDIUM)** — `.expect()` trên glyph path
   từ font người dùng → trả `Error::BadFont`.
10. **Silent effect drop (MEDIUM)** — `compose()` nuốt lỗi render → giờ
    `eprintln!` package id + error.

Docs/CI: sửa `HANDOFF.md` §7 (stale), `ROADMAP.MD` (tách MCP thành item riêng),
`release.yml`, `nix.yml`, `models.yml`, `scripts/models.py`, `docs/recipes.md`.

## Cập nhật snapshot

Copy lại `src/` (loại `target/`), chạy `cargo test -p concat-effects` +
`cargo build --release -p concat-cli`, rồi rebuild Docker image.
Không commit binary vào repo.

---

# Concat Engine (upstream README — giữ nguyên bên dưới)
